  ...
  <div class="page">
      ...
      <div class="transfer-grid">
        <article class="card transfer-card">
          ...
          <div>
            ...
            <p class="help">Для сохранения полной резервной копии сайта и переноса данных на другой сервер.</p>
          </div>
          <ul class="transfer-list">
            ...
            <li>проведённые пары, замены, корректировки учёта и отчёты решателя.</li>
          </ul>
        ...
        <article class="card transfer-card">
          ...
          <div>
            ...
            <p class="help">Выберите ранее скачанный файл <code>*.raspis.json</code> с полной резервной копией сайта.</p>
          </div>
          ...
          <template v-if="transferBundle">
            <div class="metric-row transfer-metrics">
              ...
              <div class="metric"><span>Проведено пар</span><strong>{{ transferSummary.teachingLedger }}</strong></div>
    ...
    <section v-if="tab==='facts'" class="card section">
      ...
      <div class="facts-controls">
        ...
        <label class="form-group"><span>По дату</span><input v-model="factsDateTo" type="date" class="form-input" /></label>
        <button class="btn btn-primary" :disabled="!factsFile || factsLoading" @click="previewFacts">{{ factsLoading ? 'Читаю…' : 'Проверить файл' }}</button>
      </div>
      <div v-if="factsPreview" class="preview">
        ...
        <div v-if="factsPreview.errors.length" class="notice error"><strong>Ошибки — применение заблокировано</strong><div v-for="(e,index) in factsPreview.errors.slice(0,100)" :key="index">{{ e }}</div><small v-if="factsPreview.errors.length>100">Показаны первые 100 из {{ factsPreview.errors.length }}</small></div>
        <div v-if="factsPreview.warnings.length" class="notice warning"><strong>Предупреждения</strong><div v-for="(w,index) in factsPreview.warnings.slice(0,100)" :key="index">{{ w }}</div></div>
        <div v-if="!factsPreview.imported" class="notice warning">В выбранном диапазоне не найдено занятий.</div>
        <button class="btn btn-success" :disabled="factsPreview.errors.length || !factsPreview.imported || factsCommitting" @click="commitFacts">{{ factsCommitting ? 'Сохраняю…' : 'Записать подтверждённый факт' }}</button>
      </div>
    </section>
    ...
    <section v-if="tab==='hours'" class="section">
      ...
      <div v-if="hours.schedule_found===false" class="notice warning">Расписание ещё не сгенерировано — в колонке «Проект» будет 0.</div>
      <div v-else-if="hours.projection_stale" class="notice warning">База или журнал проведённых пар изменились после генерации. Устаревший проект скрыт; запустите генерацию заново.</div>
      <div class="notice success">{{ hours.accounting_basis }}</div>
      <div v-if="hours.accounting_source_links?.length" class="notice"><strong>Источники проведённых занятий</strong><div v-for="(source,index) in hours.accounting_source_links" :key="index"><a :href="source.url" target="_blank" rel="noopener noreferrer">{{ source.label }}</a><small> {{ source.from }} — {{ source.to }}</small></div></div>
      <div v-if="hours.accounting_import_warnings?.length" class="notice warning"><strong>Требуют сверки</strong><div v-for="(warning,index) in hours.accounting_import_warnings" :key="index">{{ warning }}</div><small>Эти строки сохранены как предупреждения и не уменьшают часы другого предмета.</small></div>
      ...
      <div v-else class="table-wrap"><table><thead><tr><th>Наименование</th><th>План, ч</th><th v-if="hoursMode==='teachers'">Нагрузка, ч/нед.</th><th>Проведено, ч</th><th>Проект, ч</th><th>Зачтено, ч</th><th v-if="hoursMode==='teachers'">Замены +/−</th><th>Осталось, ч</th><th>Выполнение</th><th v-for="w in hoursMode!=='lessons' ? hours.weeks : []" :key="w.index">Нед. {{ w.index }}<small>{{ w.from }}<br>{{ w.to }}</small></th></tr></thead><tbody><tr v-for="row in pagedHourRows" :key="hourKey(row)" :class="hourStatus(row)"><td><strong>{{ hourName(row) }}</strong><button v-if="hoursMode!=='lessons'" class="workload-link" @click="openWorkload(row)">Состав нагрузки</button><small v-if="row.teacher_name&&hoursMode!=='teachers'">{{ row.teacher_name }}</small></td><td>{{ row.planned_hours }}</td><td v-if="hoursMode==='teachers'"><strong>{{ teacherWeeklyLoad(row) }}</strong><small>C ÷ {{ remainingWeeks }} нед.</small></td><td><button class="occurrence-button" :disabled="!row.scheduled_occurrences?.length" @click="openOccurrences(row,'scheduled')"><strong>{{ row.scheduled_hours }}</strong><small>{{ row.scheduled_occurrences?.length ? `Даты: ${row.scheduled_occurrences.length}` : 'дат нет' }}</small></button></td><td>{{ row.projected_hours || 0 }}</td><td><button class="occurrence-button" :disabled="hoursMode!=='teachers'||!row.credited_occurrences?.length" @click="openOccurrences(row,'credited')"><strong>{{ row.credited_hours ?? row.scheduled_hours }}</strong><small v-if="hoursMode==='teachers'">{{ row.credited_occurrences?.length ? `Факт: ${row.credited_occurrences.length}` : 'дат нет' }}</small></button></td><td v-if="hoursMode==='teachers'"><span class="positive">+{{ row.substitution_in_hours||0 }}</span> / <span class="negative">−{{ row.substitution_out_hours||0 }}</span><small v-if="row.adjustment_hours">корр.: {{ signed(row.adjustment_hours) }}</small></td><td :class="{negative:row.remaining_hours>0,positive:row.remaining_hours===0}">{{ row.remaining_hours }}</td><td><div class="progress"><span :style="{width:percent(row)+'%'}" /></div><small>{{ percent(row) }}%</small></td><td v-for="(w,i) in hoursMode!=='lessons' ? hours.weeks : []" :key="w.index"><button v-if="row.weekly_hours?.[i]" class="week-button" @click="openOccurrences(row,'scheduled',w.index)">{{ row.weekly_hours[i] }}</button><span v-else>0</span></td></tr></tbody></table></div>
    ...
    <Modal v-model="workloadModal" size="wide" :title="`Состав нагрузки — ${workloadName}`">
      ...
      <div v-else class="table-wrap workload-table"><table><thead><tr><th>Дисциплина</th><th>{{ workloadMode==='groups' ? 'Преподаватель' : 'Группа' }}</th><th>План, ч</th><th>Проведено, ч</th><th>Осталось, ч</th><th>Выполнение</th><th>Проведённые часы по датам</th></tr></thead><tbody><tr v-for="lesson in workloadLessons" :key="lesson.lesson_id" :class="hourStatus(lesson)"><td><strong>{{ lesson.name }}</strong></td><td>{{ workloadMode==='groups' ? lesson.teacher_name : lesson.group_name }}</td><td>{{ lesson.planned_hours }} ч</td><td>{{ lesson.scheduled_hours }} ч</td><td :class="{negative:lesson.remaining_hours>0,positive:lesson.remaining_hours===0}">{{ lesson.remaining_hours }} ч</td><td><div class="progress"><span :style="{width:percent(lesson)+'%'}" /></div><small>{{ percent(lesson) }}%</small></td><td><button class="occurrence-button" :disabled="!lesson.scheduled_occurrences?.length" @click="openWorkloadOccurrences(lesson)"><strong>{{ occurrenceHours(lesson.scheduled_occurrences) }} ч</strong><small>{{ lessonDatesPreview(lesson) }}</small></button></td></tr></tbody></table></div>
...
const hoursSummary=computed(()=>searchedHourRows.value.reduce((sum,row)=>{const credited=row.credited_hours??row.scheduled_hours,planned=row.planned_hours||0;sum.planned+=planned;sum.scheduled+=row.scheduled_hours||0;sum.projected+=row.projected_hours||0;sum.credited+=credited||0;sum.remaining+=row.remaining_hours||0;if(planned>0&&row.scheduled_hours===0)sum.notStarted++;if(credited>planned)sum.overrun++;return sum},{planned:0,scheduled:0,projected:0,credited:0,remaining:0,notStarted:0,overrun:0}))
...
const occurrenceHours=items=>(items||[]).reduce((sum,item)=>sum+Number(item.hours||2),0)
const latestAccountedWeek=computed(()=>(hours.value.lessons||[]).reduce((latest,lesson)=>(lesson.scheduled_occurrences||[]).reduce((value,occurrence)=>Math.max(value,Number(occurrence.week_index||0)),latest),0))
const remainingWeeks=computed(()=>Math.max(1,(hours.value.weeks?.length||1)-latestAccountedWeek.value))
const teacherWeeklyLoad=row=>(Number(row.planned_hours||0)/remainingWeeks.value).toFixed(1)
...
watch(tab,value=>{loadTabData(value);if(route.query.tab!==value)router.replace({query:{...route.query,tab:value}})})
...
function selectFactsFile(event){factsFile.value=event.target.files?.[0]||null;factsPreview.value=null}
async function previewFacts(){if(factsDateFrom.value&&factsDateTo.value&&factsDateTo.value<factsDateFrom.value){toast.error('Конечная дата раньше начальной');return}factsLoading.value=true;const raw=await api.data.get();if(!raw.ok){toast.error(raw.data?.message||'Не удалось загрузить текущие данные');factsLoading.value=false;return}try{const {parseCompletedSchedule}=await import('../utils/completedScheduleImport.js');factsPreview.value=await parseCompletedSchedule(factsFile.value,raw.data,{dateFrom:factsDateFrom.value,dateTo:factsDateTo.value})}catch(e){toast.error(`Расписание не прочитано: ${e.message}`)}finally{factsLoading.value=false}}
async function commitFacts(){factsCommitting.value=true;const check=await api.data.audit(factsPreview.value.data);if(!check.ok){factsCommitting.value=false;toast.error(check.data?.message||'Предварительный аудит не выполнен');return}const errors=(check.data.issues||[]).filter(issue=>issue.severity==='error');if(errors.length){factsCommitting.value=false;toast.error(`Импорт заблокирован аудитом: ${errors[0].message}`);return}const result=await api.data.replace(factsPreview.value.data);factsCommitting.value=false;if(result.ok){toast.success(`Подтверждено ${factsPreview.value.imported} пар. Учёт часов обновлён.`);factsPreview.value=null;loadedTabs.clear()}else toast.error(result.data?.message||'Факт не сохранён')}
